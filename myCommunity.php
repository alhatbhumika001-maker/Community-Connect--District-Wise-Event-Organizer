...
/* Only errors fixed, no major changes */
body { font-family: 'Outfit', sans-serif; background: #f8f9fa; margin: 0; color: #111827; }
.sidebar { width: 260px; position: fixed; top: 0; left: 0; bottom: 0; background: linear-gradient(90deg, #f9f2ff 0%, #d4c0ff 100%); overflow-y: auto; padding-top: 80px; border-right: 1px solid #c3b7f7; }
.sidebar-profile-img { width: 90px; height: 90px; border-radius: 50%; object-fit: cover; border: 3px solid #a076ff; box-shadow: 0 0 10px #a076ff88; display: block; margin: 0 auto 10px auto; }
.sidebar-nav .nav-link { color: #6b55a5; font-weight: 600; padding: 12px 20px; border-radius: 12px; display: block; }
.sidebar-nav .nav-link.active, .sidebar-nav .nav-link:hover { background: #d9c9ff; color: #512da8; }
.sidebar .collapse .nav-link { padding-left: 40px; }
.main { margin-left: 260px; padding: 40px 30px; }
.metric { padding: 10px; }
.metric-card { border-radius: 20px; padding: 18px; background: #ffffff; box-shadow: 0 6px 18px rgba(0, 0, 0, 0.12); transition: 0.25s ease; display: flex; flex-direction: column; justify-content: space-between; min-height: 150px; }
.metric-card:hover { transform: translateY(-4px); box-shadow: 0 10px 24px rgba(0, 0, 0, 0.16); }
.metric-top { display: flex; align-items: center; gap: 16px; }
.metric-icon-box { width: 72px; height: 72px; border-radius: 14px; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 14px rgba(123, 47, 247, 0.12); flex-shrink: 0; background: #fff; }
.bi { font-size: 2rem; color: #8540f5; }
.metric-title { font-size: 18px; font-weight: 700; color: #333; line-height: 1.05; }
.metric-count { margin-top: 14px; font-size: 28px; font-weight: 800; color: #111827; }
.community { border-radius: 12px; padding: 20px; background-color: #ffffff; box-shadow: 0 6px 18px rgba(0,0,0,0.06); margin-bottom: 30px; }
.com-banner { width: 100%; height: 160px; overflow: hidden; border-radius: 8px; margin-bottom: 12px; background: #efefef; }
.com-banner img { width: 100%; height: 100%; object-fit: cover; }
.request-box, .member { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
.request-box img, .member img { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; background: #efefef; flex-shrink: 0; }
...
.btn-outline-indigo { color: #6610f2; border: 1px solid #6610f2; }
.btn-outline-indigo:hover { background-color: #6610f2; color: #fff; }
.toggle-section { display:none; margin-top:12px; }
.request-toggle { display:flex; gap:10px; margin:10px 0; }
.toggle-btn { padding:6px 16px; border-radius:20px; border:1px solid #7450ea; background:#fff; color:#7450ea; font-weight:600; cursor:pointer; }
.toggle-btn.active { background:#7450ea; color:#fff; }
.section-title { color:#7450ea; font-weight:700; }
</style>
</head>
<body>
...
<div class="sidebar" style="margin-top:25px;">
    ...
    <nav class="sidebar-nav">
        ...
        <a class="nav-link" data-bs-toggle="collapse" href="#createMenu" role="button" aria-expanded="false" aria-controls="createMenu">Create ▾</a>
        ...
        <div class="col-12 col-md-4">
            <div class="metric-card">
                <div class="metric-top">
                    ...
                    <div class="metric-title">Total Members In Community</div>
                </div>
                <div class="metric-count" style="color: #34275f;">Total Members: <?= $res_count ?></div>
            </div>
        </div>
        <div class="col-12 col-md-4">
            <div class="metric-card">
                <div class="metric-top">
                    ...
                    <div class="metric-title">New Members Last Week</div>
                </div>
    ...
    <?php while ($row = mysqli_fetch_assoc($community_result)): ?>
        <div class="community">
            ...
            $members_stmt = $conn->prepare("
                SELECT u.user_id, u.full_name, u.username, cm.joined_at
                FROM community_members cm
                JOIN users u ON u.user_id = cm.user_id
            ...
            <div class="request-toggle">
                <button class="toggle-btn" onclick="showSection('pending<?= $community_id ?>', this)">New Requests</button>
                <button class="toggle-btn" onclick="showSection('approved<?= $community_id ?>', this)">Approved Members</button>
            </div>

            <div id="pending<?= $community_id ?>" class="toggle-section">
                <h6 class="section-title">New Requests</h6>
                    ...
                    <?php foreach($requests as $req): ?>
                        <div class="request-box">
                            ...
                            <div>
                                ...
                                <p class="username">@<?= htmlspecialchars($req['username']) ?> • <?= $req['joined_at'] ?></p>
                            </div>
                            <div class="request-actions">
                                ...
                                <a href="event_member_profile.php?id=<?= $req['user_id'] ?>" class="btn btn-sm btn-outline-primary">View Profile</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
            ...
            <div id="approved<?= $community_id ?>" class="toggle-section">
                <h6 class="section-title">Approved Members</h6>
                    ...
                    <?php while($member = $members_res->fetch_assoc()): ?>
                        <div class="member">
                            ...
                            <div>
                                ...
                                <small>Joined: <?= $member['joined_at'] ?></small>
                            </div>
                            <a href="event_member_profile.php?id=<?= $member['user_id'] ?>" class="btn btn-sm btn-outline-primary">View Profile</a>
                        </div>
                    <?php endwhile; ?>
                <?php else: ?>
